...
└── packages/
    ...
    │   ├── package.json             # miniprogram: "src"
    │   ├── scripts/                 # build.js, release.js
    │   ├── CHANGELOG.md
    │   └── src/
    ...
    ├── docs/                        # 组件文档 mini-program（dep: wedux-ui workspace:*）
...
- 组件目录：`packages/core/src/components/{name}/{name}.{js,json,wxml,scss}`

## 设计 Token

所有 token 在 `packages/core/src/styles/tokens.scss` 的 `page {}` 内定义。**不要在组件内硬编码颜色或尺寸，始终引用 token 变量。若所需 token 尚未定义，先在 tokens.scss 中补充，再在组件中引用。**
...
theme-provider 接收两个属性：`theme`（完整主题对象）和 `themeOverrides`（覆盖层）。内置 light/dark 预设定义在 `packages/core/src/components/theme-provider/presets.js`，用户 require 后传入 theme。
...
路径：`packages/docs/pages/{name}/`，包含 `{name}.{js,wxml,scss,json}` 四个文件。

- `.js`：`data.props` 数组驱动 Props 文档（name/type/default/desc）
- `.wxml`：comp-header → Props 区块 → 功能演示区块
- `.scss`：顶部 `@import '../demo.scss';`，后接页面私有样式
- `.json`：注册 `packages/core` 中的组件 + `"disableScroll": true`

## 图标使用

组件需要图标时，先检查 `packages/core/src/styles/iconfont.scss` 中是否已有所需图标。**若图标不存在，必须先提示开发者添加，确认后再继续开发。**

**组件样式隔离**：组件 SCSS 文件中使用图标时，必须在顶部引入 `@import '../../styles/iconfont.scss';`，否则因样式隔离图标字体不生效。

## 新增组件流程

1. 创建 `packages/core/src/components/{name}/` 四件套
2. 在 `packages/docs/app.json` 的 `pages` 数组加路由
3. 创建 `packages/docs/pages/{name}/` demo 页
4. 在 `packages/docs/pages/index/index.wxml` 加导航项
5. 更新本文件项目结构图
6. **表单组件**须使用 `formField` behavior（`packages/core/src/behaviors/formField.js`），不要重复实现 `_setFormItemContext`、`_getFormItem` 等方法；同时在 `packages/core/src/components/form-item/form-item.js` 顶部的 `FIELD_PATHS` 数组中添加路径
...
`packages/core/src/components/form/validator.js`：规则中含自定义 `validator` 函数时，不能通过 WXML 属性传递（setData 丢失函数引用），需用 `form.setRules()` 或 `formItem.setRule()` 方法。
