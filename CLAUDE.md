...
```plain-text
wedux-ui/                            # 根项目 = wedux-ui npm 包
├── package.json                     # name: wedux-ui，miniprogram: "src"，含发布/格式化脚本
├── pnpm-workspace.yaml              # 声明 packages-private/*
...
└── packages-private/
    ...
    └── app/                         # 门店活动运营 Demo mini-program（dep: wedux-ui workspace:*）
        ├── app.js / app.json / app.scss
        ├── project.config.json / sitemap.json / theme.json
        ├── miniprogram_npm/wedux-ui  # 组件文件副本（来自 src/）
        ├── utils/
...
路径：`packages-private/docs/pages/{name}/`，包含 `{name}.{js,wxml,scss,json}` 四个文件。

- `.js`：`data.props` 数组驱动 Props 文档（name/type/default/desc）
- `.wxml`：comp-header → Props 区块 → 功能演示区块
- `.scss`：顶部 `@import '../demo.scss';`，后接页面私有样式
- `.json`：注册 `src/components` 中的组件 + `"disableScroll": true`

## 图标使用

组件需要图标时，先检查 `src/styles/iconfont.scss` 中是否已有所需图标。**若图标不存在，必须先提示开发者添加，确认后再继续开发。**
...
2. 在 `packages-private/docs/app.json` 的 `pages` 数组加路由
3. 创建 `packages-private/docs/pages/{name}/` demo 页
4. 在 `packages-private/docs/pages/index/index.wxml` 加导航项
5. 更新本文件项目结构图
6. **表单组件**须使用 `formField` behavior（`src/behaviors/formField.js`），不要重复实现 `_setFormItemContext`、`_getFormItem` 等方法；同时在 `src/components/form-item/form-item.js` 顶部的 `FIELD_PATHS` 数组中添加路径
